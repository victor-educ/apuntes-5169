...
!!! otra "Lo que hace falta de la otra asignatura"
    Esta unidad va del 15 de diciembre al 28 de enero, en paralelo con la UT5 de 5166 (infraestructura como código con OpenTofu y Ansible, del 16 de diciembre al 29 de enero): [https://victor-educ.github.io/apuntes-5166/ut/ut5-iac/](https://victor-educ.github.io/apuntes-5166/ut/ut5-iac/).
    El entorno `pre` contra el que se lanzan las pruebas de esta unidad lo crea ese repositorio IaC en enero, y lo hace por partes: las tres VM nacen el 8 de enero, los directorios `envs/dev` y `envs/pre` quedan organizados sobre el módulo `vm` el 13, y el servicio no corre dentro hasta el 15, cuando la UT5 de Despliegue lo despliega con su playbook de Ansible. El servicio publicado allí es `api.pre.lab`, igual que en dev es `api.dev.lab`. Las tres primeras sesiones (20, 21 y 22) trabajan sobre dev, y también la sesión 23, el 14 de enero: ese día `pre` todavía no sirve la API, así que la suite funcional se lanza contra `app01` en la VPC dev, que está detrás del firewall desde la UT3, y se anota en la ficha de cada caso. A partir de la sesión 24, el 19 de enero, las pruebas de carga, rendimiento, estrés y seguridad ya van contra `pre`. `pre` es además el mismo entorno que la UT7 actualiza y la UT8 destruye con `tofu destroy`.
    Jenkins es otra cosa: **no existe hasta el 5 de febrero**, con la UT6 de Despliegue ([https://victor-educ.github.io/apuntes-5166/ut/ut6-ci/](https://victor-educ.github.io/apuntes-5166/ut/ut6-ci/), del 3 de febrero al 12 de marzo), es decir, después de la práctica evaluable de esta unidad. Por eso aquí no se monta ningún job: lo que se hace es dejar la integración **preparada y documentada** (el fragmento de `Jenkinsfile`, las credenciales que hará falta crear y los comandos exactos ya probados a mano, con su código de salida), y se conecta al pipeline en febrero, en cuanto haya controlador. Las pruebas de estas hojas se lanzan a mano desde el puesto o desde una VM de la subred front.
...
5. Publica el catálogo y enlaza cada ficha desde su regla en `alerts.yml`. Dos preparativos antes de escribir la primera anotación, para no tener que reescribir las diez fichas en febrero:
    ...
    !!! otra "Cuando llegue gitea01"
        El 12 de febrero, en la [A6.4 de Despliegue](https://victor-educ.github.io/apuntes-5166/ut/ut6-ci/),
...
- La API accesible en `app01`, dentro de la VPC dev: el entorno `pre` de 5166 no sirve la API hasta el 15 de enero, un día después de esta sesión. Donde los comandos de la hoja pongan `https://api.pre.lab`, usa `https://api.dev.lab`; la suite vale igual para los dos entornos porque la URL llega por variable.
